<template>
    <div>
        ...
        <Cell label="场所类型" required v-model="form.type" :list="placeTypeList" />
         <Cell label="场所性质" required v-model="form.prop" :list="placePropList" />
        <Cell label="产品类型" required v-model="form.tmType" :list="productList" @change="weChartTao" />
        <Cell label="扫码套餐" required v-model="form.planId" :list="weChatList" @change="planChange" />
        ...
        <Input label="收货人姓名" placeholder="请输入收货人姓名" class="border-bottom" v-required v-model="form.rcperson"></Input>
        <Input label="收货人电话" placeholder="请输入收货人电话" class="border-bottom" v-required v-model="form.rcphone"></Input>
        <Cell label="归属公司" required v-model="form.company" :list="companyList" @change="companyChange"/>
...
export default {
  ...
  data() {
    return {
      ...
      placeLevelList: [], //场所档位
      form: Object,
      companyList: [{ id: 1, name: "福建民本" }, { id: 0, name: "福州民本" }],
      placePropList:[]//场所性质
    };
  },
  created() {
  ...
  methods: {
    ...
    //地址选择回调
    mapChange({ poiname, latlng = {}, poiaddress } = {}) {
      this.form.address = poiname == "我的位置" ? poiaddress : poiname;
      this.form.latitude = latlng.lat;
      this.form.longitude = latlng.lng;
    },
    companyChange(value){
        this.$emit("company",value)
    },
    //场所性质
    async getplaceProp() {
      this.placePropList = await getDICT("placeProp");
    },
  }
};
</script>  
