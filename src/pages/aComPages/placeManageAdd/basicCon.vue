<template>
    <div>
        ...
        <Cell v-if="form.showSwitch||form.placeSwitch" label="生成物流单" isText :arrow="false">
        ...
        <Input v-if="form.billType==2" label="商家分成(%)" placeholder="请输入商家分成" @input="storeRateChange" v-required class="border-bottom" :value="form.storeRate"></Input>
        <Input v-if="form.billType==2" label="我司分成(%)" placeholder="请输入我司分成" @input="rateChange" v-required class="border-bottom" :value="form.rate"></Input>
        <Input v-if="form.billType==1" label="场地租金(元/月)" placeholder="请输入场地租金" v-required class="border-bottom" v-model="form.rent"></Input>
        <Input v-if="form.billType==2||form.billType==1" label="场地押金(元)" placeholder="请输入场地押金" class="border-bottom" v-model="form.reward"></Input>
        ...
        <!-- <Input label="开户行" placeholder="请输入开户行" class="border-bottom" v-required v-model="form.bank"></Input> -->
        <!-- BankCell -->
        ...
        <Input label="合同方名称" placeholder="请输入合同方名称" class="marginTop border-bottom" v-model="form.ctname"></Input>
        <Cell v-model="form.beginDate" label="合同开始日期" isText @rightClick="openPicker(0)" />
        <Cell v-model="form.endDate" label="合同结束日期" isText @rightClick="openPicker(1)" />
        <!-- 上传 -->
