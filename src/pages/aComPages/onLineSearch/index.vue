            ...
            <Scrolls ref="datas" :param="params" @change="scrollChange" :URL="URL" class="page-loadmore-wrapper">
                <template slot-scope="props">
                    <Row 
                        ...
                        :to="{name:'OnLineSearchInfo',query:{info:JSON.stringify(item)}}"
                        arrow >
                        <span 
                            slot="rightSub" 
                            :class="{'error-color':item.sign<15,
                            'success-color':item.sign>=15}">
                            信号强度 {{item.sign}}
                        </span>
                        <span 
                            :class="{'error-color':item.status=='离线',
                            'primary-color':item.status!=='离线'}" 
                            slot="rightTop" >
                            {{item.status}}
                        </span>    
                    </Row>
